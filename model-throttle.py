...
def get_data(csv_source,img_source,correction):
    ...
    for line in lines:  
        for i in range(3):
            ...
            measurement = [float(line[3]),(float(line[4])-float(line[5]))]

            if i==0:
                measurements['center'].append(measurement)
                images['center'].append(image)
            elif i==1:
                measurement[0] += correction
...
def flip_image(image,measurement):
    ...
    flipped_measurement = [measurement[0]*-1.0,measurement[1]]
    return flipped_image, flipped_measurement 

    
folder1 = './Training-Data/Jungle-Lap1/' 
images1, measurements1 = get_data(folder1+'driving_log.csv',folder1+'IMG/',0.20)
folder2 = './Training-Data/Jungle-Lap2/' 
images2, measurements2 = get_data(folder2+'driving_log.csv',folder2+'IMG/',0.20)        
# folder3 = './Training-Data/Jungle-Lap3/' 
# images3, measurements3 = get_data(folder3+'driving_log.csv',folder3+'IMG/',0.20)
# folder4 = './Training-Data/Jungle-Lap4/' 
# images4, measurements4 = get_data(folder4+'driving_log.csv',folder4+'IMG/',0.20)
folder5 = './Training-Data/Jungle-Lap5/' 
images5, measurements5 = get_data(folder5+'driving_log.csv',folder5+'IMG/',0.20)

original_images = []
original_measurements = []
for key in ['left','center','right']:
    original_images.extend(images1[key]+images2[key]+images5[key])
    original_measurements.extend(measurements1[key]+measurements2[key]+measurements5[key])

del images1, images2, images5
del measurements1, measurements2, measurements5 
...
for i in np.arange(-1, 1, 0.1):
    z = [(x,y) for x,y in zip(augmented_images,augmented_measurements) if (y[0]>=i and y[0]<i+0.1)]
...
model = Sequential()
model.add(Cropping2D(cropping=((60,25), (20,20)), input_shape=(160,320,3)))
...
history_object=model.fit_generator(training_generator, samples_per_epoch= len(y_train), validation_data=validation_generator, nb_val_samples=len(y_validation), nb_epoch=10, callbacks=[early_stopping], verbose=1)
model.save('model-throttle.h5')
